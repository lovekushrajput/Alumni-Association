...
function Home() {
    ...
    {/* Background Video (show on md and up). Use poster image on small screens */}
    <video
        autoPlay
        loop
        muted
        ...
        tabIndex={-1}
    >
        {/* file in public/video has spaces in filename; encode the URL */}
        <source src="/video/videoplayback%20(3).mp4" type="video/mp4" />
        {/* Fallback text for very old browsers */}
        Your browser does not support the background video.
    </video>

    {/* Mobile fallback for small screens (dark gradient) */}
    <div
        className="absolute inset-0 md:hidden bg-gradient-to-br from-slate-900 via-slate-800 to-gray-900"
        aria-hidden="true"
    />

    {/* Dark Gradient Over Video */}
    <div className="absolute inset-0 bg-gradient-to-br from-black/40 via-slate-950/50 to-black/70"></div>

    {/* Content */}
    <p className="relative z-10 bg-black/10 text-white p-8 rounded-xl text-center text-2xl shadow-lg font-semibold backdrop-blur-sm border border-white/10">
            ...
            <section className="max-w-4xl mx-auto my-12 p-8 bg-blue-50 rounded-xl shadow-md border border-blue-100">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    <div className="bg-white p-6 rounded-xl shadow border border-purple-100 flex flex-col justify-between">
                        ...
                        <Link to="/register" className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 transition text-center">
                            Register Now
                        </Link>
                    </div>
                    <div className="bg-white p-6 rounded-xl shadow border border-blue-100 flex flex-col justify-between">
                        ...
                        <Link to="/register" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition text-center">
                            Join Webinar
                        </Link>
                    </div>
                </div>
